...
# -*- coding: utf-8 -*-
"""
Visualize camera-centered CartPole target-height semantic future-attention behavior.
...
import cartpole_centered_height_semantic_fa_experiment as exp
...
def select_scenario(
    ...
    for scenario in scenarios:
        rollouts = {
            name: rollout_scenario(name, model, scenario, fa_weight, rollout_mode, fa_horizon, temperature)
...
def plot_trajectories(out_path: Path, regime: str, rollouts: Dict[str, Trajectory]) -> None:
    ...
    draw_axes(bottom_box, "Hidden cart drift x", -exp.X_THRESHOLD * 1.1, exp.X_THRESHOLD * 1.1, "x")
...
def draw_panel(draw: ImageDraw.ImageDraw, box: tuple[int, int, int, int], title: str, state: exp.CartPoleState, step: int) -> None:
    ...
    draw.text((left + 8, top + 24), f"step={step:03d}  h={exp.state_height(state):.2f}  x={state.x:+.2f}", fill="#444444")
    ...
    # Draw simple background drift marks so the camera-following view still exposes absolute drift.
    for offset in range(-3, 4):
        marker_world_x = state.x + offset * 1.0
        marker_screen_x = cart_x + (marker_world_x - state.x) * 38.0
        if left + 8 < marker_screen_x < right - 8:
            draw.line((marker_screen_x, ground_y + 6, marker_screen_x, ground_y + 14), fill="#aaaaaa", width=1)
...
def make_animation(
    ...
    for step in range(max_len):
        image = Image.new("RGB", (width, height), "white")
        draw = ImageDraw.Draw(image)
        draw.text((10, 4), "Camera-centered CartPole target-height control", fill="black")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Visualize camera-centered CartPole semantic-FA rollouts.")
    ...
    parser.add_argument("--disturbance-mode", type=str, default=exp.DEFAULT_DISTURBANCE_MODE, choices=("gaussian", "impulse"))
    ...
    parser.add_argument("--fa-horizon", type=int, default=exp.DEFAULT_FA_HORIZON)
    ...
    parser.add_argument("--png", type=str, default="cartpole_centered_height_semantic_fa_visual.png")
    parser.add_argument("--gif", type=str, default="cartpole_centered_height_semantic_fa_visual.gif")
    parser.add_argument("--json", type=str, default="cartpole_centered_height_semantic_fa_visual.json")
