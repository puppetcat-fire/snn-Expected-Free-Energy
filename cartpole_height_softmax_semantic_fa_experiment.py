...
# -*- coding: utf-8 -*-
"""
CartPole target-height maintenance with softmax semantic future attention.
...
The task is a continuing target-height variant built on standard CartPole
physical parameters rather than the original reward/termination definition.
"""
...
def stochastic_step(state: CartPoleState, action: str, rng: random.Random, noise_std: float) -> CartPoleState:
...
def collect_episode(rng: random.Random, horizon: int, noise_std: float) -> Tuple[List[CartPoleState], List[str]]:
...
def collect_dataset(seed: int, episodes: int, horizon: int, noise_std: float) -> List[Tuple[List[CartPoleState], List[str]]]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="CartPole target-height control with softmax semantic future attention.")
    ...
    parser.add_argument("--json", type=str, default="cartpole_height_softmax_semantic_fa_results.json")
    parser.add_argument("--md", type=str, default="cartpole_height_softmax_semantic_fa_results.md")
    ...
    print("=" * 120)
    print("CartPole height softmax semantic-FA experiment")
