...
# -*- coding: utf-8 -*-
"""
Camera-centered CartPole target-height maintenance with softmax semantic future attention.

This script keeps the later semantic-FA design, but moves it onto standard
CartPole physics reimplemented locally:

1. Candidate actions are scored in separate imagined branches.
2. A learned transformer world model predicts next-state continuations.
3. Future toward-target and away-from-target semantic tokens attend back to the
...
class RepresentativeCase:
    ...
    uncertainty_gated_signed_fa_action: str
...
def normalized_uncertainty_gate(logits: Sequence[float], power: float) -> float:
    if len(logits) <= 1:
        return 0.0
    probs = softmax_probs(logits, temperature=1.0)
    entropy = entropy_from_probs(probs)
    max_entropy = math.log(len(probs))
    normalized = entropy / max(1e-6, max_entropy)
    return clamp(normalized, 0.0, 1.0) ** power
...
def policy_logits(
    ...
    for action in ACTIONS:
        branches[action] = imagined_semantic_scores(
            model=model,
    ...
    base_logits = {action: branch["base"] for action, branch in branches.items()}
    uncertainty_gate = normalized_uncertainty_gate([base_logits[action] for action in ACTIONS], power=uncertainty_gate_power)
    ...
    for action in ACTIONS:
        ...
        elif policy_name == "uncertainty_gated_signed_fa":
            logits[action] = branch["base"] + fa_weight * uncertainty_gate * (branch["pos_fa"] - branch["neg_fa"])
        else:
...
def choose_action(
    ...
    logits = policy_logits(
        ...
        uncertainty_gate_power=uncertainty_gate_power,
    )
    if policy_name == "planner":
    ...
    for state in cases:
        ...
        results.append(
            RepresentativeCase(
                ...
                height=state_height(state),
                planner_action=argmax_action(planner_action_scores(state)),
                prediction_only_action=choose_action("prediction_only", model, history_states, history_actions, fa_weight, rollout_mode, fa_horizon, uncertainty_gate_power, temperature, 0.5)[0],
                positive_fa_action=choose_action("positive_fa", model, history_states, history_actions, fa_weight, rollout_mode, fa_horizon, uncertainty_gate_power, temperature, 0.5)[0],
                signed_semantic_fa_action=choose_action("signed_semantic_fa", model, history_states, history_actions, fa_weight, rollout_mode, fa_horizon, uncertainty_gate_power, temperature, 0.5)[0],
                uncertainty_gated_signed_fa_action=choose_action("uncertainty_gated_signed_fa", model, history_states, history_actions, fa_weight, rollout_mode, fa_horizon, uncertainty_gate_power, temperature, 0.5)[0],
            )
        )
    return results
...
def write_markdown(path: str, summary: Dict[str, object], cases: Sequence[RepresentativeCase]) -> None:
    ...
    lines = [
        ...
        f"- `uncertainty_gate_power = {summary['config']['uncertainty_gate_power']:.2f}`",
        "",
        "## World Model",
        "",
    ...
    for policy_name, label in (
        ...
        ("uncertainty_gated_signed_fa", "Uncertainty-gated signed FA"),
        ("planner", "Planner"),
    ):
    ...
    lines.extend(
        [
            ...
            "",
            "| X | Xdot | Theta | ThetaDot | Height | Planner | Prediction | Positive FA | Signed semantic FA | Uncertainty-gated signed FA |",
            "|---:|---:|---:|---:|---:|---|---|---|---|---|",
        ]
    )
    for case in cases:
        lines.append(
            ...
            f"{case.planner_action} | {case.prediction_only_action} | {case.positive_fa_action} | {case.signed_semantic_fa_action} | {case.uncertainty_gated_signed_fa_action} |"
        )
...
def main() -> None:
    ...
    parser.add_argument("--uncertainty-gate-power", type=float, default=1.0)
    parser.add_argument("--json", type=str, default="cartpole_centered_height_semantic_fa_results.json")
    parser.add_argument("--md", type=str, default="cartpole_centered_height_semantic_fa_results.md")
    args = parser.parse_args()

    world_model_metrics: List[WorldModelMetrics] = []
    nominal_policy_metrics: Dict[str, List[EvalMetrics]] = {
        name: [] for name in ("random", "prediction_only", "positive_fa", "signed_semantic_fa", "uncertainty_gated_signed_fa", "planner")
    }
        ...
        for policy_name in nominal_policy_metrics.keys():
            nominal_policy_metrics[policy_name].append(
                evaluate_policy(
                    ...
                    fa_horizon=args.fa_horizon,
                    uncertainty_gate_power=args.uncertainty_gate_power,
    ...
    cases = representative_cases(last_model, args.fa_weight, args.rollout_mode, args.fa_horizon, args.uncertainty_gate_power, args.policy_temperature)
    ...
    print("=" * 120)
    print("Camera-centered CartPole height softmax semantic-FA experiment")
